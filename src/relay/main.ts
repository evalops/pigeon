...
import { SlackOidcClient } from "../enrollment/slack-oidc.js";
...
const enrollment = new EnrollmentService(pool); const devices = await enrollment.loadDevices();
...
const oidc = process.env.PIGEON_SLACK_CLIENT_ID && process.env.PIGEON_SLACK_CLIENT_SECRET ? new SlackOidcClient({ clientId: process.env.PIGEON_SLACK_CLIENT_ID, clientSecret: process.env.PIGEON_SLACK_CLIENT_SECRET }) : undefined;
const store = new PostgresRelayStore(pool); const app = createRelayApp({ store, devices, enrollment, oidc, slackInternalSecret: process.env.PIGEON_SLACK_INTERNAL_SECRET, slackSigningSecret: process.env.PIGEON_SLACK_SIGNING_SECRET });
