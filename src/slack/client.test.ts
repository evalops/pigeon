...
describe("Slack client", () => {
  it("posts a redacted delegation DM with safe actions", async () => {
    ...
    const client = new SlackClient("xoxb-test", async (url, init) => { request = { url: String(url), init }; return new Response(JSON.stringify({ ok: true, channel: "D1", ts: "1.2" }), { headers: { "content-type": "application/json" } }); });
